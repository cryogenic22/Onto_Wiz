...
Contents
Lead Vision & Strategic Direction
The Problem: Why SME Knowledge Leaks
System Architecture: The Knowledge Codification Agent
Domain Pack Specification
Knowledge Capture Pipelines
Data Models & Code Scaffolding
Governance & Quality Gates
Integration with Onto_Wiz & SpecOmagic
The Agentic Loop: From Capture to Deployment
...
DecisionHeuristic
What: A codified decision rule that an SME applies intuitively — the "if X then Y unless Z" patterns.

Maps to: Onto_Wiz JudgmentPattern (signal matching + driver attribution)

Fields: trigger_signals, decision_logic, exceptions, confidence, evidence_required, typical_outcome, anti_patterns

DataQuirk
What: A known data limitation, bias, or gotcha that affects analytical accuracy.

Maps to: SpecOmagic OverrideRule (always-fire constraint) + Onto_Wiz Guardrail

Fields: data_source, quirk_description, impact_severity, workaround, validation_query, known_since, affects_metrics

ProcessPlaybook
What: A step-by-step analytical workflow with decision points, dependencies, and quality checks.

Maps to: Onto_Wiz ActionTemplate (cross-functional actions) + SpecOmagic InstructionSet (ordered rules)

Fields: steps[] (order, action, inputs, outputs, quality_check, decision_point), prerequisites, estimated_duration, common_pitfalls
...
class DataQuirk(ArtifactBase):
    ...
    def to_override_rule_dict(self) -> dict:
        ...
        return {
            ...
            "reason": f"Known limitation of {self.data_source} affecting {', '.join(self.affects_metrics)}",
        }
...
class InterviewAgent:
    ...
    def next_question(self, session: InterviewSession) -> str:
        ...
        # Determine current module and question index
    ...
    def _get_current_module(self, session: InterviewSession) -> KnowledgeModule | None:
        """Determine which module we're currently in."""
    ...
    def _get_question_index(
        ...
        """Count how many questions have been asked for this module."""
        return sum(
    ...
    def _fill_template(self, template: str, config: InterviewConfig) -> str:
        """Fill prompt template placeholders with session context."""
        replacements = {
    ...
    def _parse_extraction(self, llm_output: str, artifact_type: str) -> list[dict]:
        """Parse LLM extraction output into artifact dicts."""
        import json
        try:
    ...
    def _format_artifact_summary(self, artifact: dict) -> str:
        """Format an artifact dict for human-readable validation."""
        lines = []
    ...
    def _create_delta(self, artifact: dict, session: InterviewSession) -> str:
        """Create a Delta proposal from a confirmed artifact."""
    ...
    def _generate_closing(self, session: InterviewSession) -> str:
        """Generate session closing summary."""
        artifact_count = sum(
...
class DomainPackBuilder:
    ...
    def __init__(self, store: Any):
        """Initialise with a KnowledgeStore or LayeredStore."""
        self.store = store
    ...
    def build(
        ...
        Artifacts can be selected by:
        1. Tag filters — all artifacts matching the given tags
        2. Explicit artifact IDs — cherry-picked artifacts
        3. Both — union of both selections
        """
        artifacts = []
        ...
        # Filter to ACTIVE lifecycle only (for production packs)
        if pack_type != "draft":
            artifacts = [
    ...
    def validate(self, pack: dict) -> list[str]:
        ...
        for dep in manifest.get("dependencies", []):
            # In production, check that the dependency pack is available
            pass
        ...
        # Check that InstructionSet.applies_to references valid entities
...
class ArtifactBridge:
    ...
    This is the key integration component — it ensures that knowledge captured
    through any pipeline can flow into both systems seamlessly.
    """

    @staticmethod
    def heuristic_to_judgment_pattern(
        heuristic: Any,  # DecisionHeuristic
    ) -> dict:
    ...
    @staticmethod
    def judgment_pattern_to_heuristic(
        pattern: Any,  # JudgmentPattern
    ) -> dict:
