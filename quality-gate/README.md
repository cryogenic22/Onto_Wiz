...
**Requires:** Python 3.10+ (GitHub Actions template uses Python 3.11).
...
Optional: if your environment allows network access and you want additional lint/format hooks (ruff/mypy/eslint), use:

```bash
cp quality-gate/.pre-commit-config.extended.yaml .pre-commit-config.yaml
pre-commit install
```
...
### Export Quality CSV (Optional)

```bash
# Export per-file PRS + issue metrics (and a heuristic review score) to CSV
