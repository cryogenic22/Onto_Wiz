...
def _extract_python_exports(filepath: str) -> list[str]:
    ...
    exports: list[str] = []
    for node in ast.iter_child_nodes(tree):
        if isinstance(node, ast.ClassDef) and not node.name.startswith("_") or isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and not node.name.startswith("_"):
...
def _detect_source_globs(root: str) -> list[str]:
    ...
    for dirpath, dirnames, _ in os.walk(root):
        ...
        if current.name in ("app", "src") and rel.count("/") <= 2 or current.name == "components" and rel.count("/") <= 3:
            globs.append(f"{rel}/**")
