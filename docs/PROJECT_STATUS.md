...
| Serve door honors its own contract | 🟡 SUBMITTED | `build/F0.10` @ `fa070fa`, unreviewed, unmerged. Bundle: `docs/reviews/F0.10_REVIEW_BUNDLE.md` | **F0.10** |
...
| **F0.10** — Serve-door contract parity | **SUBMITTED, not verified.** The served L3 prompt instructs `ctx/hydrate`; the door never registered it, so the advertised consumption loop dead-ended at `unknown tool` and the only working hydrate path read a raw `.ctx` **file path** — outside the governance gate. New Tier-A `ontowiz_runtime.hydrate` hydrates strictly from `ContextResult.eligible_doc` (the same gated projection the directory is built from, so the advertised set and the hydratable set are one object); `SectionNotServableError` is one **non-leaking** refusal for gated-by-tag / gated-by-lifecycle / nonexistent alike, so the door is not an existence oracle for DRAFT content. `gate()` now enforces `ALWAYS_INCLUDED_KINDS` (previously zero non-test consumers) against the **tag** filter only — lifecycle still governs absolutely. New `ontowiz_serve.envelope` is the single trust serializer both doors call, closing the MCP omission of `artifacts_used`/`backing_deltas`; `TOOL_NAMES` is now derived from the published schema table so the lists cannot diverge. Adds MCP `ctx/hydrate` + `POST /v1/hydrate`. Spec: `docs/specs/F0-10_SERVE_DOOR_CONTRACT_PARITY.md`. | 250 passed / 1 deselected, **98.46%** cov (hydrate/envelope/mcp/api all 100%); verify-audit **gates 2–6 PASS** (CK new-code 0); **gate 1 red on one PRE-EXISTING failure** proven to fail identically at baseline `f8a79a3` — see Known debt. Blocking e2e green: `/v1/context` → section names parsed out of the returned prompt → `/v1/hydrate` → governed content. |
...
| Item | Severity | Note |
|---|---|---|
| **EOL-sensitive `pack.sig` on `commercial_analytics/0.1.0`** (was reported "stale sig blocks gate") | 🟡 **TRACKED — not a completion-gate blocker on main** | **CORRECTED 2026-07-25.** On the main tree (`foundry-build` HEAD `5ebbda3`, CRLF working copy) `test_write_results_reseals_signed_pack` **PASSES** and `verify_pack(0.1.0)` returns **True** (recomputed `_pack_digest` matches the stored seal; pytest exit 0). This entry's prior "🔴 fails / blocks the completion gate" claim was **stale** and internally contradicted by the "re-sealed → `verify_pack` True" note at line 151. **Root cause:** `_pack_digest` hashes **raw file bytes** and is **line-ending-sensitive** — `.gitattributes` declares `eol=lf` but the committed fixture files are CRLF and `pack.sig` is LF, so the committed seal matches a CRLF checkout but a fresh **LF-normalized** checkout (e.g. a detached worktree) hashes different bytes and fails. This is a **seal-robustness bug, not a stale fixture and not a governance waiver.** **Fix (tracked, S1.1/S1.2-adjacent):** canonicalize the pack's **declared text formats** (normalize line endings for known text files) before hashing — *not* indiscriminate byte normalization. This is a legitimate code fix to the seal, distinct from resealing a fixture to turn a gate green. Ref: `docs/specs/ENTERPRISE_MATURITY_CONSOLIDATION_2026-07.md` §10; `EXPERIMENT_PRE_REGISTRATION_2026-07.md`. |
