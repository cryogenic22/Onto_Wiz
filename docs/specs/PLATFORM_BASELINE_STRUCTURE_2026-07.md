...
| — S1.2 manifest-driven load (no glob, verify-before-load) | new (registry.py, release_verifier.py) | mini-spec drafted; closes #4/#5; depends S1.1 |
| — S1.3 immutable eval receipt (PackEvalSummary derived, no reseal) | new (benchmark.py, eval_contracts.py) | mini-spec drafted; closes #6/#8; depends S1.1 |
